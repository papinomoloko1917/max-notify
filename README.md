...
3. Отсекает дубли событий через общий или покамерный TTL.
4. Проверяет глобальное и покамерное временное окно отправки, если они настроены.
...
public/
  ...
  img/
    example1.png            пример настройки Dahua
    example2.png            пример настройки Dahua
...
Для каждой камеры в `/profile` можно задать свой TTL защиты от дублей в секундах. Если поле пустое, используется общий `DUPLICATE_TTL_SECONDS` из `.env`.

Пример:

```text
Входная дверь: 10 секунд
Двор: 60 секунд
```
...
Есть два уровня фильтрации по времени.

Глобальный фильтр в `.env` ограничивает отправку для всего сервиса:
...
Покамерный фильтр задается в `/profile` при добавлении или редактировании конкретной камеры. Если для камеры оба поля времени пустые, она отправляет уведомления без собственного ограничения.

Если событие пришло вне глобального или покамерного окна, сервис пишет его в лог, но не получает snapshot и не отправляет сообщение в MAX.
