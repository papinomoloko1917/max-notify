...
У каждой камеры также есть собственные поля времени:

```text
cameras.notify_allowed_from
cameras.notify_allowed_to
cameras.duplicate_ttl_seconds
```

Они задаются через `/profile` и применяются только к выбранной камере. Если `cameras.duplicate_ttl_seconds` пустой, используется общий `DUPLICATE_TTL_SECONDS`.
...
2. Приложение выбирает TTL дублей: покамерный `cameras.duplicate_ttl_seconds` или общий `DUPLICATE_TTL_SECONDS`.
3. `DuplicateGuard` проверяет, когда такой ключ был в последний раз.
4. Если ключ уже был в течение выбранного TTL, событие считается дублем.
5. Дубль пишется в лог, но snapshot и MAX не вызываются.
...
Глобальный временной фильтр стоит после защиты от дублей и до выбора камеры.

Покамерный временной фильтр применяется после выбора камеры и проверки разрешенного `rule`. Если событие пришло вне окна конкретной камеры, приложение пишет лог и возвращает:

```text
OK camera time window skipped
```

В обоих случаях snapshot и MAX не вызываются.
...
Сервер повторяет эту сборку при сохранении, поэтому данные корректно обработаются даже без JavaScript. Название камеры, snapshot URL, доступы Dahua, разрешенные `rule`, покамерное временное окно и получатели MAX берутся из MySQL.
