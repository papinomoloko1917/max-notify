...
## 1. Если на сервере уже заняты 80 и 443
...
6. Добавь камеру:
   ...
   - пароль Dahua;
   - нужные события;
   - временное окно конкретной камеры, если нужно;
   - TTL защиты от дублей конкретной камеры, если нужен;
   - клиентов, которым отправлять уведомления.

`source` формируется автоматически из названия камеры. Вручную его вводить не нужно.

Если покамерное временное окно пустое, камера отправляет уведомления в любое время, если это не запрещено глобальным фильтром `NOTIFY_ALLOWED_FROM/TO`.

Если покамерный TTL дублей пустой, используется общий `DUPLICATE_TTL_SECONDS` из `.env`.
